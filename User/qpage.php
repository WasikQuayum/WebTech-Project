...
if($_SESSION["qdif"]=="easy"){
$q="select id from  ".$_SESSION["choice"]." where dif='0' ";
$result=mysqli_query($conn,$q);

}
else{
	if($_SESSION["qdif"]=="hard"){
		$q="select id from ".$_SESSION["choice"]." where dif='1' ";
		$result=mysqli_query($conn,$q);
		
	}
	else{
		$q="select id from ".$_SESSION["choice"]." ";
		$result=mysqli_query($conn,$q);
	}
}
...
for($x=0;$x<$_SESSION["qnum"];$x++){
	$q="select * from  ".$_SESSION["choice"]." where id="."'".$id[$x]."'";
     $result=mysqli_query($conn,$q);
...
<script>
		...
		function checkscore(){
			...
			var score=0;
			var num= <?php echo $_SESSION['qnum']; ?>;
